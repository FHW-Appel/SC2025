...
class realerkenner(basisstrat):
    ...
    def act(self, history_self, history_opponent):
        ...
        if roundcounter >= 0 and roundcounter < 6:
            ...
            elif history_opponent[roundcounter - 1] == False and is_perfect_square(roundcounter - 1) == True and history_opponent[4] == False and sum(history_opponent[-9:]) == 0:
                opponent = "linus"
                #print("Gespielt gegen linus")
            elif roundcounter >= 30:
                if sum(history_opponent[0:4]) == 5 and sum(history_opponent[8:12]) == 0 and history_opponent[7] == False:
                    opponent = "TitForTwoTat"
                    #print("Gespielt gegen TitForTwoTat")
                elif history_self[roundcounter - 31:roundcounter - 1] == history_opponent[roundcounter - 30:roundcounter]:
            ...
            if opponent == "self" or opponent == "vincentvariant":
                return self.COOPERATE
            elif opponent == "erkenner" or opponent == "unknown" or opponent == "stupidniceman" or opponent == "dagoberte" or opponent == "nachtragend" or opponent == "Ping_Pong" or opponent == "bing_bong" or opponent == "linus":
                return self.DEFECT
            elif opponent == "TitForTwoTat":
                if sum(history_opponent[-6:]) == 0:
                    opponent = "TwoTitsForTat"
                    return self.COOPERATE
                elif history_self[roundcounter - 1] == True and history_self[roundcounter - 2] == False:
                    return self.DEFECT
                else:
                    return self.COOPERATE
            else:
